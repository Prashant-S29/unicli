...
>
> Planned commands: `unicli image compress`, `unicli image convert`, `unicli image resize`
> Engine: `ffmpeg` (managed via engine manager, same pattern as M2)
>
> M9 adds `cmd/image.go` and `internal/image/`. Nothing in M1–M8 is modified.
...
| **M9** | Image module | M1, M2 | `unicli image` commands |
